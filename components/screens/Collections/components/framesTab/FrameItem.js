...
const FrameItem = ({frameItem,isActive,isLocked,isSale, isCollected, setModalVisible}) => {
    ...
    const getImage = () =>{
        const img = getFrameImg(frameItem,frameItem.id)
    ...
    return (
        <SquareCard style={{
            ...
            <CollectionCardBG source={collBg} resizeMode="contain">
                ...
                <FrameImage source={getImage()}/>
                <Text setShadow={true} numberOfLines={1} madium blod center>{frameItem.name}</Text>
                <CollectButton item={frameItem}
